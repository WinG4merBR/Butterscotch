...
static void ps2Init(AudioSystem* audio, DataWin* dataWin, FileSystem* fileSystem) {
...
static void actionStop(Ps2SoundInstance* sfx, Ps2MusicStream* music, void* userData) {
    if (sfx != nullptr) sfx->active = false;
    if (music != nullptr) music->active = false;
}

static void actionPause(Ps2SoundInstance* sfx, Ps2MusicStream* music, void* userData) {
    if (sfx != nullptr) sfx->paused = true;
    if (music != nullptr) music->paused = true;
}

static void actionResume(Ps2SoundInstance* sfx, Ps2MusicStream* music, void* userData) {
...
static void ps2SetChannelCount(AudioSystem* audio, int32_t count) {
    // No-op: software mixer handles all channels internally
}

static void ps2GroupLoad(AudioSystem* audio, int32_t groupIndex) {
    // No-op: all audio is available from SOUNDS.BIN
}

static bool ps2GroupIsLoaded(AudioSystem* audio, int32_t groupIndex) {
    return true;
}
